...
**クエリ間参照はそのままでよい**：AI変換はAccess固有構文（Transform・IIF等）をSQL Server用に変換するだけで、クエリ間の参照（FROM句に他のAccessクエリ名がそのまま残る）は解決しない。`src/sql_expand.py` の `expand_query_ast()` が、SQLを一度パースしてから `exp.Table` ノードの名前を同じフォルダ内の `クエリ名` 一覧・`table.json` のテーブル名一覧と照合し、クエリ参照であれば再帰的にサブクエリとしてインライン展開する。正規表現によるテキスト置換ではなくAST上でのノード置換を行うため、文字列リテラルやドット修飾されたカラム参照（`クエリ工事集計.[工事ID]` 等）を誤って展開してしまうことはない。

例えば「クエリ工事集計」は `FROM クエリ工事基本` とだけ書けばよく、`クエリ工事基本` の中身をあらかじめサブクエリとして埋め込んでおく必要はない（後述の展開処理が自動で行う）。
...
| フィールド | 型 | 内容 |
|---|---|---|
| `テーブル名` | string | 物理テーブル名。`lineage()` のスキーマ辞書のキーになる。また `expand_query_ast()` がクエリ参照とテーブル参照を区別する際にも使われる。 |
...
| 参照クエリ1, 参照クエリ2, ... | 出力カラムの由来を辿る過程で経由したサブクエリ／CTE／クエリ参照のエイリアス名を、外側→内側の順に並べたもの。列数はそのフォルダ内で実際に現れる最大ネスト数に応じて動的に決まる（ネストがなければ1列のみ）。直接参照の場合は `参照クエリ1` に「（直接）」、それ以外の列は空欄。 |
